...
def run_pendulum_emma_optimization(output_folder=""):
    ...
    with torch.no_grad():
        ...
        maxChange = 95.0  # Match training maxChange
        getp = lambda k: predicted_params[:,:,k].mean()
        
        # Ground-truth nominal values (same as in loss function)
        alpha_nominal = 0.90  # metres
        beta_nominal = 0.05   # 1/s
        gamma_nominal = 150.0  # loss calibration for theta (adjusted to guide alpha->0.9)
        delta_nominal = 25.0   # loss calibration for omega (adjusted to guide beta->0.05)
...
def main():
    ...
    # Process multiple videos (01-05) to verify parameter variation
    video_configs = [
        ("pendulum/pendulum_90/01/video.mp4", "90_v1"),
        ("pendulum/pendulum_90/02/video.mp4", "90_v2"),
        ("pendulum/pendulum_90/03/video.mp4", "90_v3"),
        ("pendulum/pendulum_90/04/video.mp4", "90_v4"),
        ("pendulum/pendulum_90/05/video.mp4", "90_v5"),
    ]
...
class Custom_Pendulum_Loss(nn.Module):
    ...
    def forward(self):
        ...
        # Why: Explicitly guide network to learn alpha=0.9m and beta=0.05
        # What: Add squared error between learned and GT values
        alpha_gt = torch.tensor(0.90, device=dev)  # Ground truth length (m)
