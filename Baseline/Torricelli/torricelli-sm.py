...
class Custom_Torricelli_Loss(nn.Module):
    ...
    def forward(self):
        ...
        # Nominal k value from parameters.json for small torricelli
        k_nominal = 0.009553717173659406  # Nominal k value (m^(1/2)/s) - from parameters.json small
        ...
        loss_Cal_h = 0.0  # Calibrated loss value (from calibration run with maxChange=0)
        ...
        # Store raw MSE for calibration (only on first call, when loss_Cal_h is 0.0)
        if not hasattr(self, '_calibration_recorded') and loss_Cal_h == 0.0:
            self._calibration_value = raw_mse.mean().item()
...
def main():
    ...
    video_path = "../../output_selected/torricelli/small/02/video.mp4"  # Set to video path if processing from video
    weights_path = "yolo11m.pt"  # YOLO model weights
    pixel_to_meter = 0.001  # Conversion factor: adjust based on your setup (m/pixel)
    
    # Save results in small_v2 folder (like pendulum 45_v1, 90_v1, etc.)
    output_folder = "small_v2"
