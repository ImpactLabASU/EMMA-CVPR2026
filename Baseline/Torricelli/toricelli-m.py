...
class Custom_Torricelli_Loss(nn.Module):
    ...
    def forward(self):
        ...
        # Nominal k value from parameters.json for medium torricelli
        k_nominal = 0.0128264315869546  # Nominal k value (m^(1/2)/s) - from parameters.json medium
        ...
        # Calibrated value from running with maxChange=0: 0.000001
        loss_Cal_h = 0.000001  # Calibrated loss value (from calibration run with maxChange=0)
...
def main():
    ...
    video_path = "../../output_selected/torricelli/medium/03/video.mp4"  # Set to video path if processing from video
    weights_path = "yolo11m.pt"  # YOLO model weights
    pixel_to_meter = 0.001  # Conversion factor: adjust based on your setup (m/pixel)
    
    # Save results in med_v4 folder (like pendulum 45_v1, 90_v1, etc.)
    output_folder = "med_v4"
