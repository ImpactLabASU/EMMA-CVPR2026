# This is the code for rover pipeline 
...
def run_architecture_ablation():
    """
    Run architecture ablation study: LTC vs LSTM vs GRU vs Transformer.
    
    This function:
    1. Loads rover trajectory data
    2. Tests all 4 architectures
    3. Estimates 5 physical rover parameters (a, b, r, m, CM)
    ...
    print("=" * 70)
    print("ROVER ARCHITECTURE ABLATION STUDY")
    print("Comparison: LTC vs LSTM vs GRU vs Transformer")
    print("=" * 70)
    ...
    seq_len = 16
    batch_size = 2
    num_epochs = 100  # Reduced for ablation study
    ...
    for arch_name in architectures:
        print(f"\n{'='*70}")
        print(f"Testing Architecture: {arch_name.upper()}")
        print(f"{'='*70}")
        
        # Create model
        model = HarModel(model_type=arch_name, model_size=64, learning_rate=learning_rate).to(device)
        ...
        with torch.no_grad():
            ...
            a = ((1 + (0.5 - getp(0)) * maxChange / 100.0) * 0.178).item()
            b = ((1 + (0.5 - getp(1)) * maxChange / 100.0) * 0.144).item()
            r = ((1 + (0.5 - getp(2)) * maxChange / 100.0) * 0.201).item()
            m = ((1 + (0.5 - getp(3)) * maxChange / 100.0) * 26.88).item()
            CM = ((1 + (0.5 - getp(4)) * maxChange / 100.0) * 0.112).item()
            
            print(f"  Estimated: a={a:.6f}, b={b:.6f}, r={r:.6f}, m={m:.6f}, CM={CM:.6f}")
            print(f"  Training time: {training_time:.2f}s, Converged: epoch {convergence_epoch}")
            
            results.append({
                'architecture': arch_name.upper(),
                'best_loss': best_loss,
                'final_loss': train_losses[-1] if train_losses else best_loss,
    ...
    # Save results
    results_file = 'rover_architecture_ablation_results.csv'
    ...
    for arch_name in architectures:
        ...
        if arch_results:
            r = arch_results[0]
            print(f"\n{arch_name.upper()}:")
            print(f"  Loss: {r['final_loss']:.6f}")
            print(f"  Time: {r['training_time_s']:.2f}s")
            print(f"  Converged: epoch {r['convergence_epoch']}")
            print(f"  Parameters: a={r['a']:.4f}, b={r['b']:.4f}, r={r['r']:.4f}, m={r['m']:.2f}, CM={r['CM']:.4f}")


def run_EMMA_optimization(model_type="ltc"):
    """
    Main function to run EMMA rover parameter estimation.
    
    This function:
    1. Loads rover trajectory data
    2. Creates and trains the neural network
    3. Estimates 5 physical rover parameters (a, b, r, m, CM)
    4. Saves results and creates simulation visualization
    
    Args:
        model_type: Architecture to use ("ltc", "lstm", "gru", "transformer")
    """
    # Set random seeds for reproducibility
    import random
    random.seed(42)
    ...
    print(f"[STEP 3] Starting EMMA rover optimization with {model_type.upper()}...")
    ...
    model = HarModel(model_type=model_type, model_size=64, learning_rate=learning_rate).to(device)
    optimizer = model.optimizer
    scheduler = model.scheduler
    ...
    with torch.no_grad():
        ...
        # Extract 5 parameters (a, b, r, m, CM)
        a = (1 + (0.5 - getp(0)) * maxChange / 100.0) * 0.178  # X-arm length (m)
        b = (1 + (0.5 - getp(1)) * maxChange / 100.0) * 0.144   # Y-arm length (m)
        r = (1 + (0.5 - getp(2)) * maxChange / 100.0) * 0.201  # Wheel radius (m)
        m = (1 + (0.5 - getp(3)) * maxChange / 100.0) * 26.88   # Mass (kg)
        CM = (1 + (0.5 - getp(4)) * maxChange / 100.0) * 0.112  # Center of mass height (m)
        
        # Fixed parameters (not learned)
        Sf = torch.tensor(0.8, device=predicted_params.device)  # Fixed scaling factor
        
        # Save parameters to CSV (5 parameters only)
        vals = [a.item(), b.item(), r.item(), m.item(), CM.item()]
        with open('rover_coefficients.csv', 'w', newline='') as csvfile:
            ...
            for name, val, unit, desc in zip(['a', 'b', 'r', 'm', 'CM'], 
                                           vals, 
                                           ['m', 'm', 'm', 'kg', 'm'],
                                           descriptions):
                w.writerow([name, val, unit, desc])
        
        print("\n=== ESTIMATED ROVER PARAMETERS (5 Parameters) ===")
        for name, val, unit in zip(['a', 'b', 'r', 'm', 'CM'], vals, 
                                 ['m', 'm', 'm', 'kg', 'm']):
        ...
        # Simulate trajectory for the full sequence (pass 5 parameters only)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Rover EMMA Pipeline")
    parser.add_argument('--ablation', action='store_true', help='Run architecture ablation study')
    parser.add_argument('--architecture', type=str, default='ltc', choices=['ltc', 'lstm', 'gru', 'transformer'],
                       help='Architecture to use (default: ltc)')
    parser.add_argument('--skip-video', action='store_true', help='Skip video processing (use existing data)')
    ...
    if run_ablation:
        ...
        # Check if data exists, if not run video processing
        if not os.path.exists("data/xData.txt") or args.skip_video:
            if not args.skip_video:
                print("Data not found. Running video processing first...")
                video_path = "video.mp4"
                weights_path = "yolo11m.pt"
                output_video = "output/annotated_rover.mp4"
                trajectory_csv = "data/trajectory.csv"
                audio_trajectory_csv = "data/trajectory_with_audio.csv"
    ...
    try:
        ...
        print("=" * 40)
        print(f"🔄 Loading trajectory and motor data...")
        print(f"🔄 Training {args.architecture.upper()} neural network...")
        print("🔄 Estimating rover physical parameters (5 params: a, b, r, m, CM)...")
        print("🔄 Generating simulation animation...")
        run_EMMA_optimization(model_type=args.architecture)
...
class Custom_CE_Loss(nn.Module):
    ...
    def __init__(self, labels, logits, uMotor1, uMotor2, uMotor3, uMotor4):
        ...
        self.y_pred2 = logits    # [T, B, 5] - 5 rover parameters (a, b, r, m, CM)
    ...
    def forward(self):
        ...
        T, B, _ = self.y_pred2.shape  # T=timesteps, B=batch_size, 5=parameters
        ...
        # Convert normalized predictions to physical parameters (5 parameters only)
        ...
        # Fixed parameters (not learned, using nominal values)
        J = torch.tensor(0.76, device=dev)   # Moment of inertia (kg⋅m²) - fixed
        kf = torch.tensor(0.48, device=dev)  # Friction coefficient - fixed
        Cd = torch.tensor(0.1, device=dev)   # Drag coefficient - fixed
        Sf = torch.tensor(0.8, device=dev)   # Motor scaling factor - fixed
        ...
        for i in range(1, limitLoop):
            ...
            m_curr = m[t_idx]    # Mass
            CM_curr = CM[t_idx]  # Center of mass height
            # Fixed parameters (not time-varying)
            J_curr = J  # Moment of inertia (fixed)
            kf_curr = kf  # Friction coefficient (fixed)
            Cd_curr = Cd  # Drag coefficient (fixed)
            Sf_curr = Sf  # Motor scaling factor (fixed)
        ...
        # Parameter constraints for 5 learned parameters only
        # Arm length constraints (must be positive and reasonable for rover)
        param_penalty += 10.0 * torch.mean(torch.relu(-a))  # a > 0
        param_penalty += 10.0 * torch.mean(torch.relu(-b))  # b > 0
        param_penalty += 10.0 * torch.mean(torch.relu(-r))  # r > 0
        param_penalty += 5.0 * torch.mean(torch.relu(a - 0.5))  # a < 0.5m
        param_penalty += 5.0 * torch.mean(torch.relu(b - 0.4))  # b < 0.4m
        param_penalty += 5.0 * torch.mean(torch.relu(r - 0.3))   # r < 0.3m
        
        # Mass and center of mass constraints
        param_penalty += 10.0 * torch.mean(torch.relu(-m))  # m > 0
        param_penalty += 10.0 * torch.mean(torch.relu(-CM))  # CM > 0
        param_penalty += 2.0 * torch.mean(torch.relu(m - 100.0))  # m < 100kg
        param_penalty += 2.0 * torch.mean(torch.relu(CM - 0.5))  # CM < 0.5m
...
class HarModel(nn.Module):
    ...
    def __init__(self, model_type="ltc", model_size=64, learning_rate=0.001):
        ...
        # Output layer: 5 rover parameters (a, b, r, m, CM)
        self.dense = nn.Linear(model_size, 5)
    ...
    def forward(self, x):
        ...
        Returns:
            y: Predicted parameters [T, B, 5] - (a, b, r, m, CM)
        """
        ...
        T, B, H = out.shape
        y = self.sigmoid(self.dense(out.reshape(T*B, H))).reshape(T, B, 5)
        return y
...
class ThetaSimulator:
    ...
    def simulate_trajectory(self, initial_state, motor_commands, parameters, duration=10.0):
        ...
        Args:
            ...
            parameters: Learned parameters [a, b, r, m, CM] (5 parameters)
        ...
        a, b, r, m, CM = parameters[:5]
        # Fixed parameters (not learned)
        J = 0.76  # Moment of inertia
        kf = 0.48  # Friction coefficient
        Cd = 0.1  # Drag coefficient
        Sf = 0.8  # Motor scaling factor
    ...
    def create_animated_simulation(self, actual_trajectory, EMMA_trajectory):
        ...
        try:
            ...
            # Load estimated parameters (5 parameters only)
            import pandas as pd
            params_df = pd.read_csv('rover_coefficients.csv')
            ...
            m_est = params_df[params_df['Parameter'] == 'm']['Value'].iloc[0]
            CM_est = params_df[params_df['Parameter'] == 'CM']['Value'].iloc[0]
